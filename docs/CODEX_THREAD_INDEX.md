...
This index records the Codex threads that contain StealthSync implementation, debugging, and evidence history.
...
- Use the check-and-fix thread when reconstructing why a feature or security baseline change was made.
- Use the debug-config thread when investigating local startup, provider credentials, OAuth callback, or provider metadata/token behavior.
