        ...
        const fetchFiles = async () => {
            ...
            try {
                ...
                if (!cancelled) {
                    setDriveFiles([])
                    setLocalFiles([])
                    setDriveError("Failed to fetch cloud files.")
                }
            } finally {
    ...
    const executeDriveDecrypt = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                triggerLocalBanner(error?.message ?? "Decryption failed. Check your encryption key.", "error")
                return
            }
        ...
        <div className="w-100 text-white" style={{ fontFamily: "system-ui, -apple-system, sans-serif" }}>
            <div className="border-bottom border-secondary pb-2 mb-4">
                ...
                <p className="text-light-50 opacity-75 mb-0" style={{ fontSize: "0.85rem" }}>
                    Decrypt and download your files from your cloud storage platform.
                </p>
            </div>
                            ...
                            <div key={file.fileID} className="p-3 border rounded d-flex align-items-center justify-content-between gap-3" style={{ backgroundColor: "#1c1f22", borderColor: "#343a40" }}>
                                <div className="min-w-0 d-flex align-items-center gap-2">
                                    ...
                                    <div className="text-truncate ps-1">
                                        ...
                                        <div className="text-white opacity-75 mt-1" style={{ fontSize: "0.8rem" }}>
                                            {formatFileSize(file.fileSize)} | Encryption Method: {file.encMethod} | {new Date(file.uploadedAt).toLocaleDateString()}
                                        </div>
                                    </div>
                               </div>
            ...
            {driveFileToDecrypt && (
                <dialog open className="premium-modal-backdrop" onClick={() => setDriveFileToDecrypt(null)}>
                    <div className="premium-modal-surface" onClick={e => e.stopPropagation()}>
                        ...
                        <p style={{ color: "#e2e8f0", fontSize: "0.9rem", lineHeight: "1.5", marginBottom: "1.5rem" }}>
                            Are you sure you want to decrypt <span className="text-white fw-semibold">{driveFileToDecrypt.originalName}</span>? This will pull file from cloud and download to local directory.
                        </p>
                        ...
                        <div className="d-flex gap-3 justify-content-end mt-4">
                            <button 
                                ...
                                onClick={() => setDriveFileToDecrypt(null)}
                            >
                                Cancel
                            </button>
                            <button 
            ...
            {localFileToDecrypt && (
                <dialog open className="premium-modal-backdrop" onClick={() => setLocalFileToDecrypt(null)}>
                    <div className="premium-modal-surface" onClick={e => e.stopPropagation()}>
                        <div className="modal-accent-strip-alert" style={{ height: "3px", backgroundColor: "#06b6d4", marginBottom: "15px" }}></div>
                        ...
                        <p style={{ color: "#e2e8f0", fontSize: "0.9rem", lineHeight: "1.5", marginBottom: "1.5rem" }}>
                            Are you sure you want to decrypt <span className="text-white fw-semibold">{localFileToDecrypt.fileName}</span>? This will pull file from cloud and download to local directory.
                        </p>
                        ...
                        <div className="d-flex gap-3 justify-content-end mt-4">
                            <button 
                                className="btn fw-semibold px-4 py-2 border border-secondary text-white bg-transparent rounded" 
                                style={{ fontSize: "0.9rem" }}
                                onClick={() => setLocalFileToDecrypt(null)}
                            >
                                Cancel
                            </button>
                            <button 
